...
difficulty_analysis_ui <- function(id) {
  ...
  tabItem(
    ...
    fluidRow(
      box(width = 6, status = "primary", plotlyOutput(ns("difficulty_dist_chart"), height = "480px")),
      box(width = 6, status = "primary", plotlyOutput(ns("winstreak_by_difficulty_chart"), height = "480px"))
    ),
    fluidRow(
      box(width = 6, status = "primary", plotlyOutput(ns("churn_by_difficulty_chart"), height = "480px")),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$difficulty_dist_chart <- renderPlotly({
      ...
      p <- ggplot(df_counts, aes(x = labeled_difficulty, y = percentage, fill = labeled_difficulty)) +
        ...
        labs(
          title = "Distribution of Levels by Difficulty",
          subtitle = "count(labeled_difficulty) / total_levels",
          x = "Difficulty Label", y = "Percentage of Total Levels"
        ) +
    ...
    output$winstreak_by_difficulty_chart <- renderPlotly({
      ...
      p <- ggplot(df, aes(x = labeled_difficulty, y = winstreak_tier_3_rate, fill = labeled_difficulty)) +
        ...
        labs(
          title = "Tier 3 Win Streak Rate by Difficulty",
          subtitle = "winstreak_tier_3_rate",
          x = "Difficulty Label", y = "Win Streak Tier 3 Rate"
        ) +
        theme_fivethirtyeight() +
        theme(text = element_text(family = "Inter")) +
        scale_fill_manual(values = colors) +
    ...
    output$churn_by_difficulty_chart <- renderPlotly({
      ...
      p <- ggplot(df, aes(x = labeled_difficulty, y = churn_rate, fill = labeled_difficulty)) +
        geom_boxplot(alpha = 0.7) +
        labs(
          title = "Churn Rate by Difficulty",
          subtitle = "1 - (retained_players_to_next_level / players)",
          x = "Difficulty Label", y = "Churn Rate"
        ) +
        theme_fivethirtyeight() +
        theme(text = element_text(family = "Inter")) +
        scale_fill_manual(values = colors) +
    ...
    output$attempts_by_difficulty_chart <- renderPlotly({
      ...
      p <- ggplot(df, aes(x = labeled_difficulty, y = attempts_per_success, fill = labeled_difficulty)) +
        geom_boxplot(alpha = 0.7) +
        labs(
          title = "Attempts per Success by Difficulty",
          subtitle = "attempts / wins",
          x = "Difficulty Label", y = "Attempts per Success"
        ) +
        theme_fivethirtyeight() +
        theme(text = element_text(family = "Inter")) +
        scale_fill_manual(values = colors) +
